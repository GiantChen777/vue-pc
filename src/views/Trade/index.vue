  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="userAddress in trade.userAddressList"
        :key="userAddress.id"
      >
        <!-- 点击的时候，将当前点击的数据的id赋值给 isselected，这样重新渲染的时候selected的值就会改变  -->
        <span
          :class="{ username: true, selected: userAddress.id === isselected }"
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ isselect.userAddress }}</span>
        收货人：<span>{{ isselect.consignee }}</span>
        <span>{{ isselect.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    const trade = await reqGetpayDate()
    this.trade = trade
    // 2.当请求数据成功的时候，让this.isselected给赋值当前选中的地址的id
    this.isselected = trade.userAddressList.find(
      (item) => item.isDefault === '1'
    ).id
  },
  computed: {
    // 定义一个计算属性，找到当前的isselected和当前遍历出来的数据的id，就可以找到当前点击的数据，返回整个数据
    // 然后判断当前数据有没有，有的话就给返回，没有的话就给先返回一个空对象，
    isselect() {
