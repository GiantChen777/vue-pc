...
<script>
import { reqGetpaysuccess } from '@api/pay'
...
export default {
  ...
  methods: {
    async submit() {
      const result = await reqGetpaysuccess(this.$route.query.orderId)
      console.log(result)
      // 返回值是一个data，data是一个对象，可以查看接口文档13.4

      //导入element组件
      // 将获取得到的result的返回值使用  “qrcode 这个库可以将地址转化为二维码”
      // 转化成功就可以调用then和catch方法
      QRCode.toDataURL(result.codeUrl)
        .then((url) => {
          this.$alert(`<img src="${url}"/>`, '请使用微信扫码支付', {
            confirmButtonText: '我已成功支付',
            cancelButtonText: '支付中遇到了问题',
            center: true, //让最上面一行的居中显示
            showClose: false, //让右上角的x号隐藏
            showCancelButton: true, //显示取消按钮
            dangerouslyUseHTMLString: true, //这个属性可以解析上面所写的html
          })
            .then(() => {
              // 点击了成功按钮
              this.$message({
